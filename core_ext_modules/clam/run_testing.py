...
@hydra.main(
    version_base="1.2.0",
    config_path=os.path.join("config", "classification"),
    config_name="default_testing",
)
...
def single_model_evaluation(cfg):
    ...
    if not os.path.isdir(cfg.trained_model):
        raise ValueError(
            f"The path to the trained model folder does not exist. Give {cfg.trained_model}."
        )
    else:
        ...
        if not os.path.isfile(pathlib.Path(cfg.trained_model, f"hydra_config.yaml")):
            raise ValueError(
                f"The hydra configuration file for the trained model could not be found. In future implementations, the model information can be given or parsed from the trained model name."
            )
        else:
            # load the training configuration file
            training_cfg = omegaconf.OmegaConf.load(
                pathlib.Path(cfg.trained_model, f"hydra_config.yaml")
            )
    ...
    with open_dict(cfg):
        ...
        cfg.feature_level = (
            training_cfg.feature_level if "feature_level" in training_cfg else None
        )
    ...
    with open_dict(cfg):
        cfg.evaluation_results_dir = os.path.join(
            cfg.trained_model, f"evaluation_task_{cfg.task.version}"
        )
    ...
    with tqdm(total=training_cfg.k, unit="fold") as fold_tqdm:
        for ckpt_idx in range(training_cfg.k):
            ...
            if not os.path.isfile(
                pathlib.Path(cfg.trained_model, f"s_{ckpt_idx}_checkpoint.pt")
            ):
                raise ValueError(
                    f"The .pt file of the model checkpoint for the specified fold could not be found."
                )
            else:
                model_checkpoint_path = pathlib.Path(
                    cfg.trained_model, f"s_{ckpt_idx}_checkpoint.pt"
                )
            ...
            if cfg.set_to_evaluate == "merged":
                # NOTE! Here using all the data for evaluation. COncatenate the elements in the datasets
                split_dataset = dataset.return_all_splits(
                    from_id=False,
            ...
            filename = os.path.join(
                cfg.evaluation_results_dir, f"split_{ckpt_idx}_results.pkl"
            )
