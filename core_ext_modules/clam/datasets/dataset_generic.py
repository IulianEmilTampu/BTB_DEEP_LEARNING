...
class Generic_WSI_Classification_Dataset(Dataset):
    ...
    def get_merged_splits_from_df(
        self, all_splits, split_keys=["train", "val", "test"]
    ):
        """
        Utility that merges the splits from a dataframe into a single dataloader.
        """
        # stack all the splits together. Make sure that only those that are not None are concatenated
        to_concatenate = [all_splits[split] for split in split_keys]
        merged_splits = pd.concat(
            to_concatenate,
            axis=0,
        )
        merged_splits = merged_splits.reset_index(drop=True)

        if len(merged_splits) > 0:
            mask = self.slide_data["slide_id"].isin(merged_splits.tolist())
            df_slice = self.slide_data[mask].reset_index(drop=True)
            merged_splits = Generic_Split(
                df_slice, data_dir=self.data_dir, num_classes=self.num_classes
            )
        else:
            merged_splits = None
    ...
    def return_all_splits(self, from_id=True, csv_path=None):
        """
        Utility that returns all the splits into a single dataloader.
        """
        # assert csv_path
        all_splits = pd.read_csv(
            csv_path, dtype=self.slide_data["slide_id"].dtype
        ...
        merged_split = self.get_merged_splits_from_df(
            all_splits, split_keys=["train", "val", "test"]
        )
        return merged_split
