...
def seg_and_patch(
    ...
    if process_list is None:
        df = initialize_df(
            ...
            vis_params,
            patch_params,
            slide_ids=slide_df.slide_id.values.tolist() #IET
        )
    else:
        ...
        df = initialize_df(df,
                        ...
                        slide_ids=slide_df.slide_id.values.tolist()
        ) #IET
    ...
    with tqdm.tqdm(
        ...
        for i in t:
            ...
            # Inialize WSI
            wsi_object = WholeSlideImage(slide_path, spacing, backend, name=slide_id)
...
def seg_and_patch_slide(
    ...
    # Inialize WSI
    wsi_object = WholeSlideImage(Path(slide_fp), spacing, backend, name=slide_id)
