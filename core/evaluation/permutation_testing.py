...
SAVE_PATH = pathlib.Path(
    os.path.join(
        os.path.dirname(AGGREGATED_CSV_FILE), f"permutation_testing_clam_{TIME_STAMP}"
    )
)
...
models_to_compare = ["vit_conch", "vit_uni"]
...
model_aggregation_strategy = "clam_mb"
    ...
    for metric in metric_and_text.keys():
        for comparison in list_of_comparisons:
            ...
            for model in comparison:
                ...
                aus_model_df = summary_evaluation_df.loc[
                    (summary_evaluation_df.features == model)
                    & (summary_evaluation_df.classification_level == cl)
                    & (summary_evaluation_df.aggregation == model_aggregation_strategy)
                ]
                model_version_list.append(
                    aus_model_df.groupby(["aggregation"])
                    .agg({metric: "mean"})
                    .idxmax()[0]
                )
                # get the population
                populations.append(
                    np.array(
                        aus_model_df.loc[
                            aus_model_df.aggregation == model_version_list[-1]
                        ][metric]
                    )
                )
            ...
            res = permutation_test(
                ...
                n_resamples=nbr_permutations,
                alternative=alternative,
                axis=0,
            )

            # compute 95% Confidence intervals (CI)
            low_95ci_population_1, high_95ci_population_1 = st.t.interval(
                0.95,
                len(population_1) - 1,
                loc=np.nanmean(population_1),
                scale=st.sem(population_1, nan_policy="omit"),
            )
            low_95ci_population_2, high_95ci_population_2 = st.t.interval(
                0.95,
                len(population_2) - 1,
                loc=np.nanmean(population_2),
                scale=st.sem(population_2, nan_policy="omit"),
            )
...
with open(
    os.path.join(
        SAVE_PATH,
        f"table_summary_permutation_test_on_feature_extractor_aggregation_{model_aggregation_strategy}.md",
    ),
    "w",
) as f:
...
things_to_compare = ["mil", "clam_sb", "clam_mb"]
...
feature_extractors = ["vit_conch", "vit_uni"]
        ...
        for feature_extractor in feature_extractors:
            for comparison in list_of_comparisons:
                ...
                for thing_to_compare in comparison:
                    # get the model version based on the specification
                    aus_model_df = summary_evaluation_df.loc[
                        (summary_evaluation_df.aggregation == thing_to_compare)
                        & (summary_evaluation_df.classification_level == cl)
                        & (summary_evaluation_df.features == feature_extractor)
                    ]
                    model_version_list.append(
                        aus_model_df.groupby(["features"])
                        .agg({metric: "mean"})
                        .idxmax()[0]
                    )
                    # get the population
                    populations.append(
                        np.array(
                            aus_model_df.loc[
                                aus_model_df.aggregation == thing_to_compare
                            ][metric]
                        )
                    )
...
print(test_results_df.to_markdown(tablefmt="pipe", stralign="center"))
with open(
    os.path.join(SAVE_PATH, "table_summary_permutation_test_on_aggregation.md"), "w"
) as f:
    print(test_results_df.to_markdown(tablefmt="pipe", stralign="center"), file=f)
