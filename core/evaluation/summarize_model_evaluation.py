...
def check_model_integrity(trained_model_dir: str):
    ...
    if os.path.isfile(training_cfg):
        training_cfg = omegaconf.OmegaConf.load(
            pathlib.Path(trained_model_dir, f"hydra_config.yaml")
        )
        expected_nbr_folds = training_cfg.k

        # get the number of result.pkl files
        pkl_files_check = [
            os.path.isfile(pathlib.Path(trained_model_dir, f"split_{i}_results.pkl"))
            for i in range(expected_nbr_folds)
        ]
        ...
        if all(pkl_files_check):
            ...
            raise ValueError(
                f"Not all expected folds ({expected_nbr_folds}) have available results.pkl files. Missing {expected_nbr_folds - sum(pkl_files_check)}.\nModel: {trained_model_dir}"
            )
    else:
        print(
            f"Given model folder missing hydra_config.yaml file. Given {trained_model_dir}"
        )
        return None
...
@hydra.main(
    version_base="1.2.0",
    config_path=os.path.join("../../configs", "evaluation"),
    config_name="default",
)
def main(cfg: DictConfig):
    ...
    if not os.path.isdir(cfg.trained_model_dir):
        ...
        if any([".pt" in f for f in os.listdir(cfg.trained_model_dir)]):
            ...
            # process_stack.append(cfg.trained_model_dir)
        else:
            # check all the models in the folder
            for d in os.listdir(cfg.trained_model_dir):
                process_stack.append(
                    check_model_integrity(pathlib.Path(cfg.trained_model_dir, d))
                )
                # process_stack.append(cfg.trained_model_dir)
            # remove Nones
    ...
    with tqdm(total=len(process_stack), unit="model") as model_tqdm:
        for trained_model_dir in process_stack:
            ...
            if not cfg.specific_evaluation_folder:
                ...
                SAVE_PATH = Path(
                    trained_model_dir,
                    cfg.specific_evaluation_folder,
                    "summary_evaluation",
                )
                SAVE_PATH.mkdir(parents=True, exist_ok=True)
            ...
            class_fractions = [
                sum(dataset_description.label == l) / len(dataset_description)
            ...
            with tqdm(total=nbr_folds, unit="folds", leave=False) as fold_tqdm:
                for i in range(nbr_folds):
                    # open summary file
                    if not cfg.specific_evaluation_folder:
                        evaluation_file = pathlib.Path(
                            trained_model_dir, f"split_{i}_results.pkl"
                        )
                    else:
                        evaluation_file = pathlib.Path(
                            trained_model_dir,
                            cfg.specific_evaluation_folder,
                            f"split_{i}_results.pkl",
                        )
                    ...
                    probs = np.vstack(
                        [res_df.iloc[i].prob[0] for i in range(len(res_df))]
                    )
                    preds = np.argmax(probs, axis=-1)
                    labels = np.stack(
                        [res_df.iloc[i].label for i in range(len(res_df))]
                    )
                    ...
                    plotROC(
                        ...
                        PRED,
                        classes=unique_classes,
                        figure_setting=roc_figure_setting,
                        savePath=SAVE_PATH,
            ...
            for f in range(nbr_folds):
                temp_dict = {
                    ...
                    "aggregation": (
                        ...
                        if "model_type" in training_cfg.keys()
                        else "abmil"
                    ),
                    ...
                    "repetition": os.path.basename(training_cfg.task.split_dir).split(
                        "_"
                    )[-1],
                    "fold": f,
                    "set": "test",
                ...
                for c in range(nbr_classes):
                    for values, what in zip(
                        (f1_lists, prec_lists, rec_lists), ("f1", "precision", "recall")
                    ):
            ...
            cumulative_confusion_matrix = np.stack(cumulative_confusion_matrix).mean(
                axis=0
            )
            ...
            cumulative_confusion_matrix = (
                cumulative_confusion_matrix / row_sums[:, np.newaxis]
            )
            ...
            if cfg.print_single_model_summary:
                ...
                for values, what in zip(
                    (acc_list, mcc_list, averaged_AUC_list, balanced_acc),
                    ("Accuracy", "MCC", "AUC", "Balanced accuracy"),
                ):
                    median_fold_index = np.argsort(values)[len(values) // 2]
                    print(
                        f"{what:17s}: {np.mean(values):0.4f}{pm_symbol}{np.std(values):0.4f} (range [{np.min(values):0.4f}, {np.max(values):0.4f}], median at fold: {median_fold_index})"
                    )
                ...
                for c, v in label_dict.items():
                    ...
                    print(
                        f"    {c:45s}: {np.mean(values):0.4f}{pm_symbol}{np.std(values):0.4f} (range [{np.min(values):0.4f}, {np.max(values):0.4f}])"
                    )
